...
class NMF(Model): 
    ...
    ** more details to come ** 
    ...
    iso_unc : bool
        If True, account for uncertainties in the SED modeling due to
        isochrones. This uncertainty is modeled using 3 PCA components that
        have been fit to differences between various isochrone models. This
        adds three extra parameters to the SPS model.
    ...
    * only supports Calzetti+(2000) attenuation curve) and Chabrier IMF. 
    ...
    def _emu(self, tt, tage): 
        ...
        * replace redshift dependence of emulator to tage for consistency
        ...
        # apply isochrone uncertainty 
        if self._iso_unc: 
            iso_unc = (
                    theta['iso_unc0'] * self._iso_unc_pca0 + 
                    theta['iso_unc1'] * self._iso_unc_pca1 + 
                    theta['iso_unc2'] * self._iso_unc_pca2)
            lum_ssp *= np.interp(self._nmf_emu_waves, self._iso_unc_wave, np.exp(iso_unc), left=1., right=1.) 
        ...
        * 12/23/2020: age of SSPs are no longer set to the center of the
        ...
        if self._iso_unc: 
            iso_unc = (
                    ...
                    theta['iso_unc1'] * self._iso_unc_pca1 + 
                    theta['iso_unc2'] * self._iso_unc_pca2)
            lum_ssp *= np.interp(wave_rest, self._iso_unc_wave, np.exp(iso_unc), left=1., right=1.) 
    ...
    def _load_iso_unc_pca(self): 
        ''' load the 3 PCA components to model the isochrone uncertainty 
        '''
        wave    = np.load(os.path.join(os.path.dirname(os.path.realpath(__file__)), 
            'dat', 'sed_isochrone_unc.wave.npy'))
        pca     = np.load(os.path.join(os.path.dirname(os.path.realpath(__file__)),
            'dat', 'sed_isochrone_unc.pca.npy'))
        self._iso_unc_wave = wave 
        self._iso_unc_pca0 = pca[0,:]
        self._iso_unc_pca1 = pca[1,:] 
        self._iso_unc_pca2 = pca[2,:] 
        return None 
...
""" outdated classes (keeping them on file for reference --- remove soon) 
    class FSPS(Model): 
